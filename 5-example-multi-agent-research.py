...
def create_multi_agent_system():
    ...
    This demonstrates datapizza-ai's multi-agent capabilities where:
    ...
    data_analyst = Agent(
        name="data_analyst",
        client=client,
        system_prompt="""You are an expert Data Analyst with Python code execution capabilities.
...
- Provide actionable insights""",
    ...
    statistician = Agent(
        name="statistician",
        client=client,
        system_prompt="""You are an expert Statistician specializing in advanced analytics.
...
- Highlight significant findings""",
    ...
    report_writer = Agent(
        name="report_writer",
        client=client,
        system_prompt="""You are an expert Technical Writer specializing in data science reports.
...
🎯 Recommendations""",
    ...
    # ========================================================================
    # AGENT 4: MASTER ORCHESTRATOR (coordinates all agents)
    # ========================================================================
    master_orchestrator = Agent(
        name="research_director",
        client=client,
        system_prompt="""You are the Research Director coordinating a team of specialists.

YOUR TEAM:
- data_analyst: Performs data analysis and Python code execution
- statistician: Conducts advanced statistical analysis
- report_writer: Creates summaries and reports
...
3. Call data_analyst for exploratory analysis
4. Call statistician for advanced statistical tests
5. Call report_writer to synthesize findings
6. Provide final comprehensive answer

You can call other agents using their tools. Be strategic about delegation!""",
        tools=[save_finding]
    )
    ...
    # Master can call all specialized agents
    master_orchestrator.can_call([data_analyst, statistician, report_writer])
    
    # Analysts can call each other for specialized tasks
    data_analyst.can_call(statistician)
    statistician.can_call(data_analyst)
    
    # Report writer can call analysts to get more details
    report_writer.can_call([data_analyst, statistician])
...
def main():
    ...
    print(f"   • Datapizza AI - Multi-agent orchestration")
